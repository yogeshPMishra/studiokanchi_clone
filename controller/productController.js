...
exports.getAllProductsByCategory = async (req,res,next) => {
  try {
   ...
   // Getting Subcategories by categories
    const subcategories  = await Subcategory.find({category_id: category_id}) 

    // Getting The Minimum Price of Product
    const minValue = await (await Product.find({category:category_id})).sort({splprice:1}).limit(1);

    // Getting The Maximum Price of Product
    const maxValue = await (await Product.find({category:category_id})).sort({splprice:-1}).limit(1);
    ...
    arg = attr.concat(subcat, splprice, arg);
    ...
    const attributes = await Productattribute.aggregate([
      {
        $lookup: {
          from: 'products',
...
// get attributes by using product 
